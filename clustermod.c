...
double euclid2 (double** data1, int n, int index1, int index2){
  double result = 0;
  int i;
  printf("made it to euclid, index1=%d,index2=%d, n=%d\n",index1,index2,n);
  printf("\n %f",data1[index2][0]);
 for (i = 0; i < n; i++) { 
  printf("made it to euclid, index1=%d,index2=%d, n=%d\n",index1,index2,n);
     double term = data1[index1][i] - data1[index2][i];
   	result += term*term;
 }
...
double mymetric(char dist, int nc, double** data, int** mask,  double* weights, int i, int j) {
  ...
  case 'c': return correlation(nc,data,data,mask,mask,weights,i,j,0);
  case 'a': return acorrelation(nc,data,data,mask,mask,weights,i,j,0);
...
{ int i, j;
  ...
  { /* Find the pair with the shortest distance */
    ...
    distid[js] = -inode-1;
    for (i = 0; i < js; i++)
      distmatrix[js][i] = mymetric(dist,ndata,data,mask,weight,js,i);
    for (i = js + 1; i < nnodes-inode; i++)
      distmatrix[i][js] = mymetric(dist,ndata,data,mask,weight,js,i);
  }
  ...
  { const int ndata = transpose ? nrows : ncolumns;
    ...
    { result[i].distance = DBL_MAX;
      for (j = 0; j < i; j++) temp[j] =
				mymetric(dist,ndata,  data, mask, weight, i, j);
      for (j = 0; j < i; j++)
      { k = vector[j];
        if (result[j].distance >= temp[j])
        { if (result[j].distance < temp[k]) temp[k] = result[j].distance;
